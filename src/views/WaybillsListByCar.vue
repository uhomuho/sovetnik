			...
			.container.by-car
				...
				.container
					...
					.arrow.incrase(
						v-if='waybills'
						@click='incraseMonth'
						:class='waybills.listWaybill.length < 3 ? "min-height" : ""')
						img( src="@/assets/icons/incrase.svg" )
						p( v-html='nextDate')
					.arrow.decrase(
						v-if='waybills'
...
export default {
	...
	computed: {
		...
		nextDate() {
			let month = this.choosenMonth.getMonth()
			return `${month !== 11 ? monthName.calendarMonth[month+1] : monthName.calendarMonth[0]}<br>${month !== 11 ? this.choosenMonth.getFullYear() : this.choosenMonth.getFullYear() + 1}`
		},
		prevDate() {
			let month = this.choosenMonth.getMonth()
			return `${month !== 0 ? monthName.calendarMonth[month-1] : monthName.calendarMonth[11]}<br>${month !== 0 ? this.choosenMonth.getFullYear() : this.choosenMonth.getFullYear() - 1}`
		}
	},
	methods: {
		...mapActions('waybills', [
		...
		incraseMonth() {
			let month = this.choosenMonth.getMonth(),
					date = new Date(this.choosenMonth.setMonth(month+1))

			this.choosenMonth = date
